<!-- 本组件用于展示看板信息页的单箱看板完整明细，主表格只保留明细入口。 -->
...
<template>
  <PageModal :open="open" wide @close="emit('close')">
    <section v-if="kanban" class="panel kanban-detail-modal">
      ...
      <table class="table detail-table">
        <tbody>
          ...
          <tr>
            ...
            <th>出库单号</th>
            <td class="wrap-cell">
              <span v-if="outboundNos.length" class="tag-list">
                <span v-for="no in outboundNos" :key="no" class="tag-pill mono">{{ no }}</span>
              </span>
              <span v-else>-</span>
            </td>
          </tr>
          <tr>
            <th>迁移单号</th>
            <td class="wrap-cell">
              <span v-if="transferNos.length" class="tag-list">
                <span v-for="no in transferNos" :key="no" class="tag-pill mono">{{ no }}</span>
              </span>
              <span v-else>-</span>
            </td>
            <th>源看板ID</th>
